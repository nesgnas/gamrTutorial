...
public class TileManage {
    ...
    public TileManage(gamePanel gp) {
        ...
        // MAKE ARRAY HOLD TILE CAN BE ACCESS BY MANY CLASS ( GP IS THE CLASS MUST BE
        // CALL IN ALMOST CLASS)
        gp.setArr(arr);
    ...
    public void checkRoom() {
        for (int row = 0; row < 100; row++) {
            for (int col = 0; col < 100; col++) {
                ...
                // }
                if (((mapTileNum[col][row] == 118) || (mapTileNum[col][row] == 125) || (mapTileNum[col][row] == 127))
                        && (mapTileNum[col][row - 1] == 114) && (mapTileNum[col - 1][row - 1] == 115)
                        && (mapTileNum[col - 1][row] == 117)) {
                    countUpPos++;
                    findRoomUp[countUpPos][1] = col;
                    findRoomUp[countUpPos][2] = row;
                }
                if ((mapTileNum[col][row] == 118 || (mapTileNum[col][row] == 125) || (mapTileNum[col][row] == 127))
                        && (mapTileNum[col + 1][row] == 116) && (mapTileNum[col + 1][row + 1] == 108)
                        && (mapTileNum[col][row + 1] == 112)) {
                    countDownPos++;
                    findRoomDown[countDownPos][1] = col;
                    findRoomDown[countDownPos][2] = row;
                }
    ...
    public void draw(Graphics2D g2) { // draw map
        ...
        for (int i = 0; i < row1; i++) {
            if (bom[3][i] == Player.getRoomPlayerIn()) {// && bom[0][i] !=0 && bom[1][i]!=0) {
                ...
                for (Box box : boxesCopy) {
                    ...
                    // if ((x == bom[0][i] || x + 1 == bom[0][i] || x - 1 == bom[0][i]) && (y ==
                    // bom[1][i] || y + 1 == bom[1][i] || y - 1 == bom[1][i])) {
                    // if ((x == bom[0][i] && y == bom[1][i]) || (x + 1 == bom[0][i] && y ==
                    // bom[1][i]) || (x - 1 == bom[0][i] && y == bom[1][i])) {
                    if (x == bom[0][i] && y == bom[1][i]) {
                        bom[2][i] = 1;
                        bom[4][i] = nub;
                        for (int j = 0; j < row1; j++) {
                            if ((bom[4][j] == nub) && (i != j) && (bom[2][j] == 1)
                                    && bom[3][j] == Player.getRoomPlayerIn()) {
                                bom[2][j] = 0;
                                bom[4][j] = -1;
                                System.out.println("True " + bom[0][j] + " " + bom[1][j] + " " + bom[2][j] + " "
                                        + bom[3][j] + " " + bom[4][j]);
                            }
                        }
                        break;
        ...
        // else bom[2][i] = 1;
        // System.out.println(bom[0][i] + " " + bom[1][i] + " " + bom[2][i] + " " +
        // bom[3][i]);
        while (worldCol < gp.getMaxWorldCol() && worldRow < gp.getMaxWorldRow()) {
            for (int i = 0; i < count; i++) {
                if (mapTileNum[worldCol][worldRow] == 129 && !complete(Player.getRoomPlayerIn())
                ...
                } else if (mapTileNum[worldCol][worldRow] == 128 && complete(Player.getRoomPlayerIn())
                        && gate_pos(worldCol, worldRow) == Player.getRoomPlayerIn()) {
                    for (int j = 0; j < count; j++)
                        if (arr[j] == 129) {
             ...
             * while (worldCol < gp.getMaxWorldCol() && worldRow < gp.getMaxWorldRow()) {
             * for (int i = 0; i < count; i++) {
             * if (door_press && mapTileNum[worldCol][worldRow] == 128 && gate_pos(worldCol,
             * worldRow) == Player.getRoomPlayerIn())
             * for (int j =0;j<= count;j++){
             * if (arr[j] == 129){
             * mapTileNum[worldCol][worldRow] = 129;
             * tileNum = j;
             ...
             * }
             * else
             * if (!door_press && mapTileNum[worldCol][worldRow] == 129 &&
             * gate_pos(worldCol, worldRow) == Player.getRoomPlayerIn())
             * for (int j =0;j<= count;j++){
             * if (arr[j] == 128){
             * mapTileNum[worldCol][worldRow] = 128;
    ...
    public boolean complete(int numroom) {
         ...
         * {
         * for (Box box : boxes) {
         * int x = box.solidArea.x/gp.getTitleSize();
         * int y = box.solidArea.y/gp.getTitleSize();
         * //System.out.println(box.getPosX() + " + " + box.getPosY());
         * if ((x == bom[0][i] || x + 1 == bom[0][i] || x - 1 == bom[0][i])
         * && (y == bom[1][i] || y + 1 == bom[1][i] || y - 1 == bom[1][i])) {
         * bom[2][i] = 1;
         * break;
         * }
         * }
         * if (bom[2][i]==1)
         * System.out.println("True " + bom[0][i] + " " + bom[1][i] + " " + bom[2][i] +
         * " " + bom[3][i]);
         * // if (bom[2][i] == 0) {
         * // compl = false;
         * // break;
